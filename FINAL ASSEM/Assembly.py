...
class assembler:
    ...
    def addi(self, line):
        ...
        reg1, reg2, imm_str = parts[0], parts[1], parts[2]

        if reg1 not in globals() or reg2 not in globals():
            print('Error: No such register')
            return

        imm = int(imm_str)
        imm_binary = decimaltobinary(imm, 'signed', 12)
        reg2_val = binarytodecimal(globals()[reg2], 'signed')
        result = reg2_val + imm
        if result > (2**31 - 1) or result < -(2**31):
            print('Error: Illegal immediate length')
            return

        globals()[reg1] = decimaltobinary(result, 'signed', 32)
        globals()['pc'] += 4
        output.write(imm_binary + registers[reg2] + '000' + registers[reg1] + '0010011\n')


    def add(self, line):
        globals()['prev'] = globals()['pc']
        regs = line.split()[1].split(',')
        
        if any(reg not in globals() for reg in regs):
            print('Error: No such register')
            return
        
        reg_dest, reg_src1, reg_src2 = regs
        val_src1 = binarytodecimal(globals()[reg_src1], 'signed')
        val_src2 = binarytodecimal(globals()[reg_src2], 'signed')
        
        result = val_src1 + val_src2
        if result > (2**31 - 1) or result < -(2**31):
            print('Error: Illegal immediate length')
            return

        globals()[reg_dest] = decimaltobinary(result, 'signed')
        globals()['pc'] += 4
        output.write('0000000' + registers[reg_src2] + registers[reg_src1] + '000' + registers[reg_dest] + '0110011\n')
    ...
    def sub(self, line):
        ...
        globals()[reg_dest] = decimaltobinary(val_src1 - val_src2, 'signed')
        globals()['pc'] += 4
        output.write('0100000' + registers[reg_src2] + registers[reg_src1] + '000' + registers[reg_dest] + '0110011\n')
    ...
    def sll(self, line):
        ...
        globals()[reg_dest] = decimaltobinary(val_src1 << val_src2, 'signed')
        globals()['pc'] += 4
        output.write('0000000' + registers[reg_src2] + registers[reg_src1] + '001' + registers[reg_dest] + '0110011\n')
    ...
    def srl(self, line):
        ...
        globals()[reg_dest] = decimaltobinary(val_src1 >> val_src2, 'signed')
        globals()['pc'] += 4
        output.write('0000000' + registers[reg_src2] + registers[reg_src1] + '101' + registers[reg_dest] + '0110011\n')
...
def bgeu(self, line):
    ...
    if operands[0] not in globals() or operands[1] not in globals():
        print('Error:No such register')
        return

    if operands[2]+':' not in globals()['labels'] and not contains_integers(operands[2]):
        print('Error:Invalid immediate')
        return

    reg1=globals()[operands[0]]
    reg2=globals()[operands[1]]
    imm=operands[2]

    if operands[2]+':' in globals()['labels']:
        imm_value=labels[operands[2]+':'] * 4-(globals()['pc'] + 4)
        imm=imm_value if binarytodecimal(reg1) >= binarytodecimal(reg2) else int(imm)

    try:
        imm_bin=decimaltobinary(int(imm), 'signed', 12)
        globals()['pc']=globals()['pc'] + int(imm) + 4
        output.write(imm_bin[0] + imm_bin[2:8] + registers[operands[1]] + registers[operands[0]] + '111' + imm_bin[8:] + imm_bin[1] + '1100011\n')
    except OverflowError:
        print('Error:Illegal immediate length')
    ...
    try:
        imm_bin=decimaltobinary(int(imm), 'signed', 12)
        globals()['pc']=globals()['pc'] + int(imm) + 4
        output.write(imm_bin[0] + imm_bin[2:8] + registers[operands[1]] + registers[operands[0]] + '100' + imm_bin[8:] + imm_bin[1] + '1100011\n')
    except OverflowError:
        print('Error:Illegal immediate length')
    ...
    def bltu(self, line):
        ...
        if operands[2]+':' not in labels and not contains_integers(operands[2]):
            print('Error: Invalid immediate')
            return
        ...
        try:
            imm_bin = decimaltobinary(int(imm), 'signed', 12)
            globals()['pc'] = globals()['pc'] + int(imm) + 4
            output.write(imm_bin[0] + imm_bin[2:8] + registers[operands[1]] + registers[operands[0]] + '110' + imm_bin[8:] + imm_bin[1] + '1100011\n')
        except OverflowError:
            print('Error: Illegal immediate length')
    ...
    def auipc(self, line):
        ...
        if regs[0] not in globals():
            print('Error: No such register')
            return

        imm_str = regs[1]
        if not contains_integers(imm_str):
            print('Error: Invalid immediate')
            return
        ...
        try:
            imm_binary = decimaltobinary(imm, 'signed', 20)
            globals()[regs[0]] = decimaltobinary(globals()['pc'] + imm1, 'signed')
            globals()['pc'] += 4
            output.write(imm_binary + registers[regs[0]] + '0010111\n')
        except OverflowError:
            print('Error: Illegal immediate length')
    ...
    def lui(self, line):
        ...
        try:
            imm_binary = decimaltobinary(imm, 'signed', 20)
            globals()[regs[0]] = decimaltobinary(imm1, 'signed')
            globals()['pc'] += 4
            output.write(imm_binary + registers[regs[0]] + '0010111\n')
        except OverflowError:
            print('Error: Illegal immediate length')
    ...
    def jal(self, line):
        ...
        imm_str = regs[1]
        if not contains_integers(imm_str) and imm_str+':' not in globals()['labels']:
            print('Error: Invalid immediate')
            return

        if imm_str+':' not in globals()['labels']:
            imm = int(imm_str)
            try:
                imm1 = decimaltobinary(imm, 'signed', 20)
                globals()[regs[0]] = decimaltobinary(globals()['pc']+4, 'signed')
                globals()['pc'] += imm + 4
                output.write(imm1[0] + imm1[10:] + imm1[9] + imm1[1:9] + registers[regs[0]] + '0010111\n')
            except OverflowError:
                globals()['prev'] = globals()['pc']
                print('Error: Illegal immediate length')
                return
        elif imm_str+':' in globals()['labels']:
            try:
                imm = decimaltobinary(labels[imm_str+':']*4 - (globals()['pc']+4), 'signed', 20)
                globals()[regs[0]] = decimaltobinary(globals()['pc']+4, 'signed')
                globals()['pc'] = labels[imm_str+':']*4
                output.write(imm[0] + imm[10:] + imm[1:9] + registers[regs[0]] + '0010111\n')
            except OverflowError:
                print('Error: Illegal immediate length')
                return
